...
import com.nhn.minidooray.gateway.domain.response.RestPageImpl;
...
public class AccountApiServiceImpl implements AccountApiService {
    ...
    @Override
    public List<Account> getAccountsById(String[] ids) {
        ApiResultResponse<Account> resultResponse = ApiCallUtil.get(new ParameterizedTypeReference<>() {
        }, restTemplate, urlPrefix + accountApiMappingProperties.getReadAccountsById() + "?ids=" + String.join(",", ids));

        if (resultResponse.isEmpty()) {
            throw new NoSuchException("Accounts Not Found");
        }
    ...
    public Page<Account> getAccounts(Pageable pageable) {
        ApiResultResponse<RestPageImpl<Account>> resultResponse = ApiCallUtil.get(new ParameterizedTypeReference<>() {
        }, restTemplate, urlPrefix + accountApiMappingProperties.getReadAccountList() + "?page=" + pageable.getPageNumber() + "&size=" + pageable.getPageSize());
